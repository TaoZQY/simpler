...
from tests.ut.py.test_kernel_mode_c_api import (
    _binaries,
    _build_eager_callable,
    kernel_close_faults,  # noqa: F401
)
...
SCENARIOS = (
    ...
    "graph_recreate",
    "long_chain",
    "prepare_first_global",
    "prepare_first_thread_local",
    "prepare_second_global",
    "prepare_second_thread_local",
)
...
def test_hbg_kernel_capture(st_platform, st_device_ids, scenario, intermediate, kernel_close_faults):  # noqa: F811
...
def _run(platform, device, scenario, intermediate):
    ...
    callable_id = -1 if scenario.startswith("prepare_first") else worker.kernel_prepare_callable(chip)
    ...
    def record(nodes):
        nonlocal callable_id, committed, captured_nodes
        graph = ctypes.c_void_p()
        mode = int(scenario.endswith("thread_local"))
        _check(lib.aclmdlRICaptureBegin(caller.stream, mode), "capture begin")
        if scenario.startswith("prepare_"):
            if scenario.startswith("prepare_second"):
                launch(*nodes[0])
            previous_callable = callable_id
            callable_id = _prepare_without_sync(worker, chip)
            assert callable_id >= 0 and callable_id != previous_callable
            committed = worker.committed_device_memory()
    ...
    def replay(graph):
        nonlocal replays
        if captured_nodes and replays == 0:
            # A callable prepared in capture is usable before that graph's first replay.
            cid, tensors, _ = captured_nodes[0]
            launch(cid, tensors, 17.0)
            sync()
            io.verify(tensors[1], [float(i % 127) + 17.0 for i in range(128 * 128)])
...
def _prepare_without_sync(worker, chip):
    observer = ctypes.CDLL(None)
    observer.arm_prepare_guard.argtypes = []
    observer.arm_prepare_guard.restype = None
    observer.finish_prepare_guard.argtypes = []
    observer.finish_prepare_guard.restype = ctypes.c_int
    observer.arm_prepare_guard()
    try:
        return worker.kernel_prepare_callable(chip)
    finally:
        assert observer.finish_prepare_guard() == 0, "HBG prepare performed a blocking synchronization"
