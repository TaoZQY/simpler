...
TEST(SharedMemoryInit, PerRingHeapSizesStoredInHeader) {
    DeviceArena arena;
    uint64_t task_window_sizes[PTO2_MAX_RING_DEPTH] = {64, 64, 64, 64};
    uint64_t heap_sizes[PTO2_MAX_RING_DEPTH] = {4096, 8192, 16384, 32768};
    uint64_t sm_size = PTO2SharedMemoryHandle::calculate_size_per_ring(task_window_sizes);
    const size_t off_handle = arena.reserve(sizeof(PTO2SharedMemoryHandle), alignof(PTO2SharedMemoryHandle));
    const size_t off_buffer = arena.reserve(static_cast<size_t>(sm_size), PTO2_ALIGN_SIZE);
    ...
    ASSERT_TRUE(handle->init_per_ring(buffer, sm_size, task_window_sizes, heap_sizes));
    for (int r = 0; r < PTO2_MAX_RING_DEPTH; r++) {
        EXPECT_EQ(handle->header->rings[r].task_window_size, task_window_sizes[r]);
        EXPECT_EQ(handle->header->rings[r].heap_size, heap_sizes[r]);
    }
}
