...
struct NativeFake {
    ...
    void initialize() {
        fixture.initialize();
        auto header = fixture.packet;
        header.payload_bytes = 16;
        std::memcpy(packet.data(), &header, sizeof(header));
        fixture.binding.packet = {reinterpret_cast<const uint8_t *>(packet.data()), sizeof(packet)};
        ...
        native.runtime_binding = this;
        native.validate_runtime_binding = [](void *p, const KernelInvocationBinding &,
                                             const KernelResourceBinding &) noexcept {
            return static_cast<NativeFake *>(p)->validation_error;
        };
    }
    KernelLaunchResult launch(void *caller = ptr(100)) {
        return launch_bound_kernel_native(fixture.state, fixture.binding, native, caller);
    }
};
...
TEST(KernelNativeBinder, AicpuFailureCancelsWithSingleAllOnesFillAndRestoresPlaceholder) {
    ...
    EXPECT_EQ(f.fixture.state.phase(), KernelContextPhase::Poisoned);
}
...
TEST(KernelNativeBinder, InvalidPreparedArgumentsRejectBeforeAnyEnqueue) {
    for (int fault = 0; fault < 10; ++fault) {
        ...
        EXPECT_EQ(f.fixture.state.phase(), KernelContextPhase::ReadyEnqueued);
    }
}
