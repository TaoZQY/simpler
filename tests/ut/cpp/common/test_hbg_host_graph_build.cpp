...
protected:
    ...
    int32_t upload() { return hbg::upload_graph(&runtime, &api, rt, host_arena, layout, result); }
};
...
TEST_F(HostGraphBuildTest, KernelRequirementsMatchDeviceRegionsWithoutUploading) {
    ASSERT_EQ(build(chain_entry), 2);
    hbg::KernelResourceRequirements requirements;
    ASSERT_EQ(hbg::get_kernel_resource_requirements(result, layout, requirements), 0);
    const auto contract = requirements.pipeline_contract();
    ...
    uint64_t total = 0;
    ASSERT_TRUE(requirements.runtime_owned_bytes(total));
    EXPECT_EQ(
...
TEST_F(HostGraphBuildTest, KernelRequirementSnapshotSurvivesAnotherBuild) {
    ASSERT_EQ(build(empty_entry), 0);
    hbg::KernelResourceRequirements first;
    ASSERT_EQ(hbg::get_kernel_resource_requirements(result, layout, first), 0);
    const uint64_t first_image_bytes = first.runtime_arena_bytes;
    ASSERT_EQ(build(chain_entry), 2);
    hbg::KernelResourceRequirements second;
    ASSERT_EQ(hbg::get_kernel_resource_requirements(result, layout, second), 0);
...
TEST_F(HostGraphBuildTest, KernelRequirementsRejectFailedBuildAndSizeOverflow) {
    hbg::KernelResourceRequirements requirements{1, 2, 3, 4};
    EXPECT_EQ(hbg::get_kernel_resource_requirements(result, layout, requirements), PTO_RUNTIME_ERR_INVALID_STATE);
    EXPECT_EQ(requirements.gm_heap_bytes, 1u);
    EXPECT_LT(build(fatal_entry), 0);
    EXPECT_EQ(hbg::get_kernel_resource_requirements(result, layout, requirements), PTO_RUNTIME_ERR_INVALID_STATE);
    ASSERT_EQ(build(chain_entry), 2);
    result.image_bytes = UINT64_MAX;
    EXPECT_EQ(hbg::get_kernel_resource_requirements(result, layout, requirements), PTO_RUNTIME_ERR_CAPACITY_EXCEEDED);
...
TEST(HbgKernelRequirements, IncludesSeparateBlocksAndRejectsTotalOverflow) {
    hbg::KernelResourceRequirements requirements{4096, 8192, 256, 512};
    uint64_t total = 0;
    ASSERT_TRUE(requirements.runtime_owned_bytes(total));
    EXPECT_EQ(total, 13056u);
    requirements.graph_definition_bytes = UINT64_MAX;
    EXPECT_FALSE(requirements.runtime_owned_bytes(total));
    EXPECT_EQ(total, 13056u);
...
TEST_F(HostGraphBuildTest, ConcurrentKernelQueriesPublishIndependentSnapshots) {
    ASSERT_GE(build(graph_entry), 0);
    hbg::KernelResourceRequirements expected;
    ASSERT_EQ(hbg::get_kernel_resource_requirements(result, layout, expected), 0);
    auto query = [&]() {
        hbg::KernelResourceRequirements output;
        const auto status = hbg::get_kernel_resource_requirements(result, layout, output);
