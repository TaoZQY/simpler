...
Environment overrides:
  ...
  PTO_ISA_ROOT           910B PTO ISA root. Default: $CANN_HOME/pto-isa
  VENV_DIR               Python venv. Default: <simpler-main>/.venv
...
_setup_910_main() {
  ...
  if [[ "${chip}" == "910b" ]]; then
    ...
    if [[ -z "${PTO_ISA_ROOT:-}" ]]; then
      export PTO_ISA_ROOT="${cann_home}/pto-isa"
    fi
  ...
  echo "INFO:   DEVICE=3 CMD=\"./tools/benchmark_rounds.sh -p a2a3 -d 3 -r tensormap_and_ringbuffer -n 100 -v\" ./tools/run_pipeline_all.sh --block-dim ${SIMPLER_BLOCK_DIM}"
