...
int run_prepared(
    ...
    int enable_dep_gen, const char *output_prefix, PtoRunTiming *out_timing
) {
    if (out_timing != NULL) {
    ...
    try {
        ...
        int32_t resolved_strategy = resolve_pipeline_strategy_with_env(pipeline_strategy);
        r->pipeline_defer_submit_disabled = resolve_pipeline_defer_submit_disabled_with_env();
        PipelineLayout pipeline_layout = resolve_pipeline_layout(resolved_strategy);
        LOG_INFO_V0(
            "Pipeline strategy: id=%d name=%s sched_threads=%d orch_threads=%d", resolved_strategy,
            pipeline_layout.name, pipeline_layout.scheduler_threads, pipeline_layout.orchestrator_threads
        );
        set_runtime_pipeline_strategy(r, resolved_strategy);
