...
void l2_perf_aicpu_record_orch_counter_phase(
    AicpuPhaseId phase_id, uint64_t timestamp, uint32_t submit_idx, uint64_t task_id, uint32_t extra1, uint32_t extra2
) {
    if (s_orch_thread_idx < 0 || s_phase_header == nullptr) return;
    l2_perf_aicpu_record_phase(s_orch_thread_idx, phase_id, timestamp, timestamp, submit_idx, task_id, extra1, extra2);
}
