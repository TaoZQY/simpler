...
int L2PerfCollector::export_swimlane_json() {
    ...
    if (l2_perf_level_ >= L2PerfLevel::SCHED_PHASES) {
        ...
        auto orch_phase_name = [](AicpuPhaseId id) -> const char * {
            ...
            case AicpuPhaseId::ORCH_PIPELINE_COUNTERS:
                return "orch_pipeline_counters";
            case AicpuPhaseId::ORCH_O1_FOUR_STAGE_COUNTERS:
                return "orch_o1_four_stage_counters";
            case AicpuPhaseId::ORCH_O1_SUBMIT_COUNTERS_A:
                return "orch_o1_submit_counters_a";
            case AicpuPhaseId::ORCH_O1_SUBMIT_COUNTERS_B:
                return "orch_o1_submit_counters_b";
            case AicpuPhaseId::ORCH_O1_PREPARE_COUNTERS:
                return "orch_o1_prepare_counters";
            case AicpuPhaseId::ORCH_O1_ACTIVITY_COUNTERS:
                return "orch_o1_activity_counters";
            default:
                return "unknown";
            }
        };
        ...
        if (has_orch_phases) {
            ...
            for (size_t t = 0; t < collected_phase_records_.size(); t++) {
                ...
                for (const auto &pr : collected_phase_records_[t]) {
                    ...
                    if (pr.phase_id == AicpuPhaseId::ORCH_PIPELINE_COUNTERS) {
                        outfile << ", \"compact_deferred_count\": " << pr.extra1
                                << ", \"deferred_commit_count\": " << pr.extra2;
                    } else if (pr.phase_id == AicpuPhaseId::ORCH_O1_FOUR_STAGE_COUNTERS) {
                        outfile << ", \"counter_kind\": " << pr.task_id
                                << ", \"first_cycles\": " << pr.extra1 << ", \"second_cycles\": " << pr.extra2;
                    } else if (pr.phase_id == AicpuPhaseId::ORCH_O1_SUBMIT_COUNTERS_A ||
                               pr.phase_id == AicpuPhaseId::ORCH_O1_SUBMIT_COUNTERS_B ||
                               pr.phase_id == AicpuPhaseId::ORCH_O1_PREPARE_COUNTERS) {
                        outfile << ", \"counter_kind\": " << pr.task_id
                                << ", \"first_cycles\": " << pr.extra1 << ", \"second_cycles\": " << pr.extra2;
                    } else if (pr.phase_id == AicpuPhaseId::ORCH_O1_ACTIVITY_COUNTERS) {
                        outfile << ", \"counter_kind\": " << pr.task_id
                                << ", \"first_value\": " << pr.extra1 << ", \"second_value\": " << pr.extra2;
                    }
                    outfile << "}";
                    first = false;
                }
