 ...
 *                        SCHED_DISPATCH and selected orchestrator phases define
 *                        nonzero meanings; other phases pass 0.
 */
...
/**
 * Record a one-shot orchestrator counter phase.
 *
 * Uses the orchestrator's dedicated buffer slot and carries run-level counters
 * in AicpuPhaseRecord::extra1/extra2 without changing the hot submit_task()
 * phase-record ABI.
 */
void l2_perf_aicpu_record_orch_counter_phase(
    AicpuPhaseId phase_id, uint64_t timestamp, uint32_t submit_idx, uint64_t task_id, uint32_t extra1 = 0,
    uint32_t extra2 = 0
);
