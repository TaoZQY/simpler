...
static void resolve_ring_heap_sizes(Runtime *runtime) {
    uint64_t default_heap_sizes[PTO2_MAX_RING_DEPTH] = {
        PTO2_HEAP_SIZE,
        PTO2_HEAP_SIZE,
        PTO2_HEAP_SIZE * 2,
        PTO2_HEAP_SIZE,
    };
    uint64_t uniform_heap_size = runtime->heap_size ? runtime->heap_size : 0;
    bool has_per_ring_override = false;

    for (int r = 0; r < PTO2_MAX_RING_DEPTH; r++) {
        char env_name[32];
        snprintf(env_name, sizeof(env_name), "PTO2_RING_HEAP_R%d", r);
        uint64_t override_size = parse_env_uint64(env_name, 1024, true);
        if (override_size) {
            runtime->ring_heap_sizes[r] = override_size;
            has_per_ring_override = true;
        } else {
            runtime->ring_heap_sizes[r] = uniform_heap_size ? uniform_heap_size : default_heap_sizes[r];
        }
    }

    if (has_per_ring_override) {
        LOG_INFO_V0(
            "Per-ring heap sizes: r0=%" PRIu64 " r1=%" PRIu64 " r2=%" PRIu64 " r3=%" PRIu64,
            runtime->ring_heap_sizes[0], runtime->ring_heap_sizes[1], runtime->ring_heap_sizes[2],
            runtime->ring_heap_sizes[3]
        );
    }
}
...
extern "C" int bind_prepared_to_runtime_impl(
    ...
    // Keep essential orch_start/orch_active timing lines but suppress detailed
    // post-handoff diagnostics for log-overhead A/B runs.
    {
        const char *env_val = std::getenv("PTO2_SUPPRESS_ORCH_DIAG_LOGS");
        if (env_val && (env_val[0] == '1' || env_val[0] == 't' || env_val[0] == 'T')) {
            runtime->suppress_orch_diag_logs = true;
        }
    }
    ...
    // Read ring buffer size overrides from environment
    {
        ...
        resolve_ring_heap_sizes(runtime);
        ...
        LOG_INFO_V0(
            "Ring heap sizes: r0=%" PRIu64 " r1=%" PRIu64 " r2=%" PRIu64 " r3=%" PRIu64,
            runtime->ring_heap_sizes[0], runtime->ring_heap_sizes[1], runtime->ring_heap_sizes[2],
            runtime->ring_heap_sizes[3]
        );
    }
    ...
    for (int r = 0; r < PTO2_MAX_RING_DEPTH; r++) {
        total_heap_size += runtime->ring_heap_sizes[r];
    }
