...
- **Thread model**: baseline uses scheduler threads plus one orchestrator
  thread; 2O pipeline layouts use O1 for async scope release, while 4O
  layouts use O1/O2/O3 as descriptor/fanin/publish workers
...
| `PLATFORM_MAX_AICPU_THREADS` | 6 | A3 AICPU thread budget for scheduler and orchestrator roles |
...
1. **INPUT/INOUT**: `PTO2TensorMap::lookup` first checks a no-false-negative
   address filter populated by prior inserts. If the tensor base address has
   never been registered, lookup returns immediately; otherwise it searches for
   overlapping producers.
...
| 2 | Initialize slot state and materialize payload tensors/scalars needed by the orchestration return value |
...
| 5 | **Commit**: 2O layouts keep task descriptor/fanin/wiring publication synchronous on O0; 4O layouts pipeline descriptor writes on O1, fanin metadata and producer `fanout_count` updates on O2, and wiring publication on O3. |
| 6 | **Push to wiring queue**: O0 in 2O layouts, or O3 in 4O layouts, pushes to global `PTO2SpscQueue`; scheduler thread 0 asynchronously wires fanout edges (lock + dep_pool + early_finished check + ready push) |
| 7 | **Scope release**: in 2O layouts, non-empty `scope_end` is queued to O1 after synchronous task commits; other layouts flush pending commit records before releasing scope references. |
...
The orchestrator pushes each submitted task to the global `scheduler->wiring_queue` (a wait-free SPSC queue). Scheduler thread 0 drains this queue in batches, deferring if the queue holds fewer than a full batch of items to reduce contention (unless a final flush is needed at end of execution). Ready tasks produced by one wiring drain are grouped by resource shape and pushed to the global ready queues with `push_batch`, reducing per-task ready-queue atomics. For each task:
...
With unset baseline `aicpu_thread_num=4`, the AICPU runs 4 threads:

| Thread | Role | Cores |
| ------ | ---- | ----- |
| 0 | Scheduler | AICore clusters `0,3,6...` |
| 1 | Scheduler | AICore clusters `1,4,7...` |
| 2 | Scheduler | AICore clusters `2,5,8...` |
| 3 | Orchestrator | none |

Core assignment is cluster-aligned round-robin: one AICore cluster is
`1 AIC + 2 AIV`, and cluster `ci` is owned by scheduler `ci % 3`.

With `pipeline_strategy=0`, `pipeline_strategy=1`, or `pipeline_strategy=2`
and `aicpu_thread_num=4`, the executor uses separate 4-thread debug/tuning
layouts:

- strategy 0: cluster0=`O0 O1`, cluster1=`S0 S1`
- strategy 1: cluster0=`S0 S1`, cluster1=`O0 O1`
- strategy 2: cluster0=`O0 O1`, cluster1=`S0 S1`

Strategy 0 is the default debug module used by the base-vs-strategy0 run. It
currently copies strategy 2's 2O+2S cross-cluster behavior, but keeps a
separate enum, name, and executor predicate so strategy0-only edits do not
change strategy2. Strategy 1 remains the fixed 2S2O tuning strategy. Strategy 2
keeps the same submit/fanin/wiring behavior as strategy 1 but places O and S on
different AICPU clusters.

With `pipeline_strategy=3..4` and `aicpu_thread_num=6`, the executor uses the
selected larger Scheduler / Orchestrator-role thread layout:

- strategy 3: cluster0=`O0 O1 O2 O3`, cluster1=`S0 S1`
- strategy 4: cluster0=`S0 S1 O2 O3`, cluster1=`O0 O1`

With `pipeline_strategy=5` and `aicpu_thread_num=4`, the executor keeps the
same 2S2O role layout as strategy 1 but enables O1 to consume full submit
commit records from the O0 queue.

Scheduler internals still see contiguous scheduler indexes (`S0..S{N-1}`).
The primary orchestrator still owns task allocation and output
materialization. Strategies 0, 1, and 2 are independent submit-split branches:
O0 materializes payload/descriptor and writes compact deferred metadata for
the O0-to-O1 task batch boundary; O1 computes deferred dependencies, fanin,
and wiring publication, then emits a scheduler-drain hint so S0 can consume
small/final wiring batches without changing ready-queue or dispatch semantics.
Strategies 0 and 2 keep that behavior while moving O0/O1 and S0/S1 onto
different AICPU clusters; strategy0 is reserved as the debug module. In
strategy 5, O1 consumes queued full submit commit records.
In 4O layouts O1 writes task
descriptors, O2 writes fanin metadata, and O3 publishes to the wiring queue;
`scope_end` still uses the conservative flush path. Device logs print a
separate wall time for each secondary orchestrator worker so the balance of
the split is visible directly.
