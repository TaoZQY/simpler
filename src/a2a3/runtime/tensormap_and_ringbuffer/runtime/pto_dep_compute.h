...
[[nodiscard]] inline bool compute_task_fanin_impl(
    ...
    for (int32_t i = 0; i < tensor_count; i++) {
        ...
        if (tensor_map.lookup_skip_reason(*tensor) != PTO2TensorMapLookupSkipReason::NONE) {
            continue;
        }
...
compute_task_fanin(const DepInputs &inputs, PTO2TensorMap &tensor_map, bool in_manual_scope, Emit emit) {
    return compute_task_fanin_impl(
        inputs.tensor_count, [&](int32_t i) { return inputs.tensors[i].ptr; },
        [&](int32_t i) { return inputs.arg_types[i]; }, tensor_map, in_manual_scope, emit
    );
}

template <typename Emit>
[[nodiscard]] inline bool
compute_payload_task_fanin(const PTO2TaskPayload &payload, PTO2TensorMap &tensor_map, bool in_manual_scope, Emit emit) {
    return compute_task_fanin_impl(
        payload.tensor_count, [&](int32_t i) { return &payload.tensors[i]; },
        [&](int32_t i) { return static_cast<TensorArgType>(payload.arg_tags[i]); }, tensor_map, in_manual_scope, emit
    );
}
...
register_task_outputs(const DepInputs &inputs, PTO2TaskId task_id, PTO2TensorMap &tensor_map, bool in_manual_scope) {
    register_task_outputs_impl(
        inputs.tensor_count, [&](int32_t i) { return inputs.tensors[i].ptr; },
        [&](int32_t i) { return inputs.arg_types[i]; }, task_id, tensor_map, in_manual_scope
    );
}
...
inline void register_payload_task_outputs(
    ...
    register_task_outputs_impl(
        payload.tensor_count, [&](int32_t i) { return &payload.tensors[i]; },
        [&](int32_t i) { return static_cast<TensorArgType>(payload.arg_tags[i]); }, task_id, tensor_map,
        in_manual_scope
    );
}
