...
PTO2Runtime *runtime_create_from_sm(
    ...
    return runtime_create_from_sm_per_ring(
        mode, sm_base, sm_size, task_window_size, gm_heap, heap_sizes, arena, dep_pool_capacity
    );
}

PTO2Runtime *runtime_create_from_sm_per_ring(
    PTO2RuntimeMode mode, void *sm_base, uint64_t sm_size, uint64_t task_window_size, void *gm_heap,
    const uint64_t heap_sizes[PTO2_MAX_RING_DEPTH], DeviceArena &arena, int32_t dep_pool_capacity
) {
    ...
    uint64_t task_window_sizes_u64[PTO2_MAX_RING_DEPTH];
    for (int r = 0; r < PTO2_MAX_RING_DEPTH; r++) {
        task_window_sizes_u64[r] = task_window_size;
    }
    if (!rt->sm_handle->init_per_ring(sm_base, sm_size, task_window_sizes_u64, heap_sizes)) {
        arena.release();
        return nullptr;
    }
    ...
    rt->gm_heap_size = 0;
    for (int r = 0; r < PTO2_MAX_RING_DEPTH; r++) {
        rt->gm_heap_size += heap_sizes[r];
    }
    rt->gm_heap_owned = false;

    if (!rt->orchestrator.init_from_layout_per_ring(orch_layout, arena, rt->sm_handle->header, gm_heap, heap_sizes)) {
        arena.release();
        return nullptr;
    }
