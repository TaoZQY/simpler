...
#if PTO2_PROFILING
struct PTO2TaskAllocProfile {
    uint64_t task_wait_cycles{0};
    uint64_t heap_wait_cycles{0};
    uint64_t task_wait_spins{0};
    uint64_t heap_wait_spins{0};
...
public:
...
#if PTO2_PROFILING
        ,
        PTO2TaskAllocProfile *profile = nullptr
#endif
    ) {
...
#if PTO2_PROFILING
            uint64_t wait_iter_start = profile != nullptr ? get_sys_cnt_aicpu() : 0;
#endif
            spin_count++;
#if PTO2_ORCH_PROFILING
            if (!waiting) {
...
#if PTO2_PROFILING
            if (profile != nullptr) {
                uint64_t wait_iter_cycles = get_sys_cnt_aicpu() - wait_iter_start;
                if (blocked_on_heap) {
                    profile->heap_wait_cycles += wait_iter_cycles;
                    profile->heap_wait_spins++;
                } else {
                    profile->task_wait_cycles += wait_iter_cycles;
                    profile->task_wait_spins++;
                }
    ...
    bool allocation_would_block_on_heap(int32_t output_size) {
        ...
        return !can_bump_heap(aligned_size);
    }
...
private:
    ...
    bool can_bump_heap(uint64_t alloc_size) const {
        uint64_t top = heap_top_;
        if (alloc_size == 0) {
            return true;
        }
        ...
        if (top >= tail) {
            uint64_t space_at_end = heap_size_ - top;
            return space_at_end >= alloc_size || tail > alloc_size;
        }
