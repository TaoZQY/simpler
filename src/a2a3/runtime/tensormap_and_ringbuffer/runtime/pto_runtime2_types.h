...
struct PTO2TaskPayload {
    ...
    uint8_t arg_tags[MAX_TENSOR_ARGS];
    ...
    void init(const Arg &args, TaskOutputTensors &result, PTO2TaskAllocResult &alloc_result, PTO2OutputLayout &layout) {
        ...
        for (int32_t i = 0; i < args.tensor_count(); i++) {
            ...
            arg_tags[i] = static_cast<uint8_t>(tag);
...
static_assert(offsetof(PTO2TaskPayload, arg_tags) == 536, "arg tags must fit in metadata padding");
