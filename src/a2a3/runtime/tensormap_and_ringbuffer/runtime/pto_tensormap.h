...
constexpr int32_t PTO2_TENSORMAP_ADDR_FILTER_WORDS = 4;
constexpr int32_t PTO2_TENSORMAP_RANGE_FILTER_SLOTS = 64;
constexpr int32_t PTO2_TENSORMAP_RANGE_FILTER_UNSAFE_WORDS = 4;
constexpr int32_t PTO2_TENSORMAP_EXACT_CACHE_SLOTS = 64;

enum class PTO2TensorMapLookupSkipReason : uint8_t {
    NONE = 0,
    EMPTY_ADDR = 1,
    RANGE = 2,
};

struct PTO2TensorMapRangeFilterSlot {
    uint64_t addr{0};
    uint64_t max_end{0};
    int32_t version{0};
...
struct PTO2TensorMapExactCacheSlot {
    uint64_t addr{0};
    uint64_t start{0};
    uint64_t end{0};
    int32_t version{0};
    DataType dtype{DataType::FLOAT16};
    PTO2TensorMapEntry *entry{nullptr};
    bool active{false};
    bool disabled{false};
...
struct alignas(64) PTO2TensorMapEntry {
    ...
    OverlapStatus check_overlap(const Tensor &input) const {
        ...
        if (input.is_contiguous && is_contiguous && input.ndims == 1 && ndims == 1 && input.dtype == dtype &&
            input.strides[0] == strides[0] && strides[0] == 1) {
            return in_range_bytes.contains(ent_range_bytes) ? OverlapStatus::COVERED : OverlapStatus::OTHER;
        }
...
struct PTO2TensorMap {
    ...
    uint64_t addr_filter[PTO2_TENSORMAP_ADDR_FILTER_WORDS]{};
    uint64_t range_filter_unsafe[PTO2_TENSORMAP_RANGE_FILTER_UNSAFE_WORDS]{};
    PTO2TensorMapRangeFilterSlot range_filter[PTO2_TENSORMAP_RANGE_FILTER_SLOTS]{};
    PTO2TensorMapExactCacheSlot exact_cache[PTO2_TENSORMAP_EXACT_CACHE_SLOTS]{};
    ...
    uint32_t addr_filter_bit(uint64_t addr) const {
        constexpr uint32_t bit_count = PTO2_TENSORMAP_ADDR_FILTER_WORDS * 64;
        addr *= 0x9E3779B97F4A7C15ULL;
        return static_cast<uint32_t>(addr >> (64 - __builtin_ctz(bit_count)));
    }

    bool addr_may_have_entry(uint64_t addr) const {
        uint32_t bit = addr_filter_bit(addr);
        return (addr_filter[bit >> 6] & (1ULL << (bit & 63))) != 0;
    }

    void mark_addr_has_entry(uint64_t addr) {
        uint32_t bit = addr_filter_bit(addr);
        addr_filter[bit >> 6] |= 1ULL << (bit & 63);
    }

    uint32_t range_filter_slot(uint64_t addr) const {
        addr *= 0xD6E8FEB86659FD93ULL;
        return static_cast<uint32_t>(addr >> (64 - __builtin_ctz(PTO2_TENSORMAP_RANGE_FILTER_SLOTS)));
    }

    uint32_t exact_cache_slot(uint64_t addr, uint64_t start, uint64_t end) const {
        uint64_t key = addr ^ (start * 0x9E3779B97F4A7C15ULL) ^ (end * 0xD6E8FEB86659FD93ULL);
        key *= 0xA24BAED4963EE407ULL;
        return static_cast<uint32_t>(key >> (64 - __builtin_ctz(PTO2_TENSORMAP_EXACT_CACHE_SLOTS)));
    }

    bool is_range_filter_eligible(const Tensor &tensor) const {
        return tensor.version == 0 && tensor.is_contiguous && tensor.ndims == 1 && tensor.strides[0] == 1;
    }

    bool is_range_filter_eligible(const PTO2TensorMapEntry &entry) const {
        return entry.version == 0 && entry.is_contiguous && entry.ndims == 1 && entry.strides[0] == 1;
    }

    uint32_t range_filter_unsafe_bit(uint64_t addr, DataType dtype) const {
        constexpr uint32_t bit_count = PTO2_TENSORMAP_RANGE_FILTER_UNSAFE_WORDS * 64;
        uint64_t key = addr ^ (static_cast<uint64_t>(dtype) * 0xA24BAED4963EE407ULL);
        key *= 0x9FB21C651E98DF25ULL;
        return static_cast<uint32_t>(key >> (64 - __builtin_ctz(bit_count)));
    }

    bool range_filter_is_unsafe(uint64_t addr, DataType dtype) const {
        uint32_t bit = range_filter_unsafe_bit(addr, dtype);
        return (range_filter_unsafe[bit >> 6] & (1ULL << (bit & 63))) != 0;
    }

    void mark_range_filter_unsafe(uint64_t addr, DataType dtype) {
        uint32_t bit = range_filter_unsafe_bit(addr, dtype);
        range_filter_unsafe[bit >> 6] |= 1ULL << (bit & 63);
    }

    bool range_filter_key_matches(const PTO2TensorMapRangeFilterSlot &slot, uint64_t addr, int32_t version,
                                  DataType dtype) const {
        return slot.active && slot.addr == addr && slot.version == version && slot.dtype == dtype;
    }

    bool exact_cache_key_matches(const PTO2TensorMapExactCacheSlot &slot, const Tensor &tensor,
                                 uint64_t end) const {
        return slot.active && !slot.disabled && slot.addr == tensor.buffer.addr &&
               slot.version == tensor.version && slot.dtype == tensor.dtype && slot.start == tensor.start_offset &&
               slot.end == end;
    }

    bool exact_cache_key_matches(const PTO2TensorMapExactCacheSlot &slot, const PTO2TensorMapEntry &entry,
                                 uint64_t end) const {
        return slot.active && !slot.disabled && slot.addr == entry.buffer_addr &&
               slot.version == entry.version && slot.dtype == entry.dtype && slot.start == entry.start_offset &&
               slot.end == end;
    }

    void update_range_filter_on_insert(const PTO2TensorMapEntry &entry) {
        uint32_t slot_index = range_filter_slot(entry.buffer_addr);
        PTO2TensorMapRangeFilterSlot &slot = range_filter[slot_index];
        const bool key_matches = range_filter_key_matches(slot, entry.buffer_addr, entry.version, entry.dtype);
        if (!key_matches) {
            slot.addr = entry.buffer_addr;
        ...
        if (!is_range_filter_eligible(entry)) {
            mark_range_filter_unsafe(entry.buffer_addr, entry.dtype);
            slot.disabled = true;
            return;
        }

        if (!slot.disabled) {
            const uint64_t end = entry.start_offset + entry.effective_extent_elem();
            if (end > slot.max_end) {
                slot.max_end = end;
            }
        }
    ...
    void update_exact_cache_on_insert(
        const PTO2TensorMapEntry &entry, bool addr_may_have_prior_entry, bool replace_exact_cache
    ) {
        if (range_filter_is_unsafe(entry.buffer_addr, entry.dtype)) {
            return;
        }

        const uint64_t end = entry.start_offset + entry.effective_extent_elem();
        uint32_t slot_index = exact_cache_slot(entry.buffer_addr, entry.start_offset, end);
        PTO2TensorMapExactCacheSlot &slot = exact_cache[slot_index];
        const bool exact_matches = exact_cache_key_matches(slot, entry, end);
        if (!is_range_filter_eligible(entry)) {
            if (slot.active && slot.addr == entry.buffer_addr && slot.dtype == entry.dtype) {
                slot.disabled = true;
                slot.entry = nullptr;
            }
            return;
        ...
        if (exact_matches) {
            if (replace_exact_cache || slot.pending_replace) {
                slot.entry = const_cast<PTO2TensorMapEntry *>(&entry);
                slot.pending_replace = false;
            } else {
                slot.entry = nullptr;
                slot.disabled = true;
                slot.pending_replace = false;
        ...
        bool monotonic_disjoint = !addr_may_have_prior_entry;
        const PTO2TensorMapRangeFilterSlot &range_slot = range_filter[range_filter_slot(entry.buffer_addr)];
        if (!monotonic_disjoint &&
            range_filter_key_matches(range_slot, entry.buffer_addr, entry.version, entry.dtype) &&
            !range_slot.disabled) {
            monotonic_disjoint = range_slot.max_end <= entry.start_offset;
        }
        if (!monotonic_disjoint) {
            mark_range_filter_unsafe(entry.buffer_addr, entry.dtype);
            slot.addr = entry.buffer_addr;
    ...
    void note_exact_cache_entry_removed(const PTO2TensorMapEntry &entry) {
        if (!is_range_filter_eligible(entry)) {
            return;
        }
        const uint64_t end = entry.start_offset + entry.effective_extent_elem();
        PTO2TensorMapExactCacheSlot &slot = exact_cache[exact_cache_slot(entry.buffer_addr, entry.start_offset, end)];
        if (!exact_cache_key_matches(slot, entry, end) || slot.entry != &entry) {
            return;
        }
        slot.entry = nullptr;
        slot.pending_replace = true;
    }

    bool range_filter_excludes_overlap(const Tensor &tensor) const {
        if (!is_range_filter_eligible(tensor)) {
            return false;
        }
        if (range_filter_is_unsafe(tensor.buffer.addr, tensor.dtype)) {
            return false;
        }
        const PTO2TensorMapRangeFilterSlot &slot = range_filter[range_filter_slot(tensor.buffer.addr)];
        if (!range_filter_key_matches(slot, tensor.buffer.addr, tensor.version, tensor.dtype) || slot.disabled) {
            return false;
        }
        return slot.max_end <= tensor.start_offset;
    }

    PTO2TensorMapLookupSkipReason lookup_skip_reason(const Tensor &tensor) const {
        if (!addr_may_have_entry(tensor.buffer.addr)) {
            return PTO2TensorMapLookupSkipReason::EMPTY_ADDR;
        }
        if (range_filter_excludes_overlap(tensor)) {
            return PTO2TensorMapLookupSkipReason::RANGE;
        }
        return PTO2TensorMapLookupSkipReason::NONE;
    }

    bool lookup_exact_cached(const Tensor &tensor, PTO2TensorMapEntry *&entry, OverlapStatus &overlap_status) const {
        if (!is_range_filter_eligible(tensor) || range_filter_is_unsafe(tensor.buffer.addr, tensor.dtype)) {
            return false;
        }
        const uint64_t end = tensor.start_offset + tensor.extent_elem();
        const PTO2TensorMapExactCacheSlot &slot =
            exact_cache[exact_cache_slot(tensor.buffer.addr, tensor.start_offset, end)];
        if (!exact_cache_key_matches(slot, tensor, end) || slot.entry == nullptr) {
            return false;
        }

        PTO2TensorMapEntry *candidate = slot.entry;
        if (candidate->bucket_index == -1 || !entry_valid(*candidate)) {
            return false;
        }
        if (candidate->buffer_addr != tensor.buffer.addr || candidate->version != tensor.version ||
            candidate->dtype != tensor.dtype || candidate->start_offset != tensor.start_offset ||
            !is_range_filter_eligible(*candidate)) {
            return false;
        }
        const uint64_t candidate_end = candidate->start_offset + candidate->effective_extent_elem();
    ...
    void lookup(const Tensor &tensor, Fn &&on_match) {
        ...
        g_lookup_count++;
#endif
        if (lookup_skip_reason(tensor) != PTO2TensorMapLookupSkipReason::NONE) {
            return;
        }
     ...
     */
    void insert(const Tensor &tensor, PTO2TaskId producer_task_id, bool replace_exact_cache = false) {
        PTO2TensorMapEntry *entry = new_entry();
        entry->copy_from_tensor(tensor);
        link_entry(entry, tensor.buffer.addr, producer_task_id, replace_exact_cache);
    }
     ...
     */
    void link_entry(PTO2TensorMapEntry *entry, uint64_t addr, PTO2TaskId producer_task_id, bool replace_exact_cache) {
#if PTO2_TENSORMAP_PROFILING
        g_insert_count++;
#endif
        const bool addr_may_have_prior_entry = addr_may_have_entry(addr);
        mark_addr_has_entry(addr);
        update_exact_cache_on_insert(*entry, addr_may_have_prior_entry, replace_exact_cache);
        update_range_filter_on_insert(*entry);
    ...
    void remove_entry(PTO2TensorMapEntry &entry) {
        note_exact_cache_entry_removed(entry);
