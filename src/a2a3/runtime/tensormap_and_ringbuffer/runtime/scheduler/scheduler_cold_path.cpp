...
int32_t SchedulerContext::init(
    ...
    if (is_l2_swimlane_enabled()) {
        ...
        if (l2_perf_level_ >= L2PerfLevel::SCHED_PHASES) {
            int32_t phase_sched_threads = orch_to_sched_ ? (thread_num_ - 1) : sched_thread_num_;
            l2_perf_aicpu_init_phase(runtime->worker_count, phase_sched_threads);
        }
    }
#endif
...
void SchedulerContext::on_orchestration_done(
    ...
    if (completed_.load(std::memory_order_acquire)) {
        ...
        int32_t transition_wait_threads = thread_num_ - 1;
        // Wait for every thread that will enter dispatch after transition.
        while (wait_reassign_.load(std::memory_order_acquire) != transition_wait_threads) {
            if (completed_.load(std::memory_order_acquire)) {
                break;
            }
            SPIN_WAIT_HINT();
