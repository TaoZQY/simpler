...
struct PTO2ProducerReleaseDetail {
    uint64_t producer_release_cycles{0};
    uint64_t ring_advance_cycles{0};
    uint32_t release_count{0};
    uint32_t consumed_count{0};
    uint32_t advance_attempt_count{0};
    uint32_t advance_success_count{0};
};
...
struct PTO2SchedulerState {
    ...
    //   3. drain hints — orchestrator write, thread 0 read
    ...
    int drain_wiring_queue(bool force_drain = false) {
        int wired = 0;
        PTO2TaskSlotState *ready_batches[PTO2_NUM_RESOURCE_SHAPES][WiringState::BATCH_SIZE];
        int ready_counts[PTO2_NUM_RESOURCE_SHAPES] = {};
        PTO2TaskSlotState *dummy_batch[WiringState::BATCH_SIZE];
        int dummy_count = 0;
        ...
        if (wiring.batch_index >= wiring.batch_count) {
            ...
            // unless force_drain, a drain hint, or backoff limit reached.
            if (!force_drain && wiring.queue.size() < WiringState::BATCH_SIZE) {
                if (!has_drain_hint && !wiring.orch_needs_drain.load(std::memory_order_acquire) &&
        ...
        while (wiring.batch_index < wiring.batch_count) {
            ...
            wiring.batch_index++;
            PTO2TaskSlotState *ready_slot = wire_task(rss, ws, wfanin);
            if (ready_slot != nullptr) {
                PTO2ResourceShape shape = ready_slot->active_mask.to_shape();
                if (shape == PTO2ResourceShape::DUMMY) {
                    dummy_batch[dummy_count++] = ready_slot;
                } else {
                    int shape_idx = static_cast<int32_t>(shape);
                    ready_batches[shape_idx][ready_counts[shape_idx]++] = ready_slot;
                }
            }
            wired++;
        }

        for (int shape_idx = 0; shape_idx < PTO2_NUM_RESOURCE_SHAPES; shape_idx++) {
            ready_queues[shape_idx].push_batch(ready_batches[shape_idx], ready_counts[shape_idx]);
        }
        dummy_ready_queue.push_batch(dummy_batch, dummy_count);
        return wired;
    }
     ...
     */
    PTO2TaskSlotState *wire_task(RingSchedState &rss, PTO2TaskSlotState *ws, int32_t wfanin) {
        PTO2TaskPayload *wp = ws->payload;
        ws->fanin_count = wfanin + 1;
        PTO2TaskSlotState *ready_slot = nullptr;
    ...
    void release_producers(
        PTO2TaskSlotState **slot_states, int32_t count, uint64_t &atomic_count,
        PTO2ProducerReleaseDetail *detail = nullptr
    ) {
        bool ring_needs_advance[PTO2_MAX_RING_DEPTH] = {};
        for (int32_t i = 0; i < count; i++) {
            uint64_t release_start = detail != nullptr ? get_sys_cnt_aicpu() : 0;
            PTO2TaskSlotState &slot_state = *slot_states[i];
            int32_t new_refcount = slot_state.fanout_refcount.fetch_add(1, std::memory_order_acq_rel) + 1;
            atomic_count += 1;  // fanout_refcount.fetch_add
            if (new_refcount != slot_state.fanout_count) {
                if (detail != nullptr) {
                    detail->producer_release_cycles += get_sys_cnt_aicpu() - release_start;
            ...
            if (!slot_state.task_state.compare_exchange_strong(
                ...
                atomic_count += 1;  // failed task_state CAS
                if (detail != nullptr) {
                    detail->producer_release_cycles += get_sys_cnt_aicpu() - release_start;
                    detail->release_count++;
                }
            ...
            atomic_count += 1;  // successful task_state CAS
        ...
        for (int32_t ring_id = 0; ring_id < PTO2_MAX_RING_DEPTH; ring_id++) {
            ...
            uint64_t advance_start = detail != nullptr ? get_sys_cnt_aicpu() : 0;
            if (detail != nullptr) {
                detail->advance_attempt_count++;
            }
            int32_t expected_lock = 0;
            if (ring_sched_states[ring_id].advance_lock.compare_exchange_strong(
                    expected_lock, 1, std::memory_order_acquire, std::memory_order_relaxed
                ...
                atomic_count += 2;  // try-lock CAS + unlock store
                if (detail != nullptr) {
                    detail->advance_success_count++;
                }
            } else {
                atomic_count += 1;  // failed try-lock CAS
            }
            if (detail != nullptr) {
                detail->ring_advance_cycles += get_sys_cnt_aicpu() - advance_start;
            }
        }
    }
#endif
    ...
    void on_scope_end(
        PTO2TaskSlotState **task_slot_states, int32_t count, PTO2ProducerReleaseDetail *detail = nullptr
    ) {
        ...
        release_producers(task_slot_states, count, g_orch_scope_end_atomic_count, detail);
#else
        (void)detail;
        if (count > 0) __builtin_prefetch(task_slot_states[0], 1, 0);
        for (int32_t i = 0; i < count; i++) {
        ...
        release_producers(task_slot_states, count);
#endif
    }
