...
bool PTO2TensorMap::init_from_layout(const PTO2TensorMapLayout &layout, DeviceArena &arena) {
    ...
    for (int32_t i = 0; i < PTO2_TENSORMAP_ADDR_FILTER_WORDS; i++) {
        addr_filter[i] = 0;
    }
    for (int32_t i = 0; i < PTO2_TENSORMAP_RANGE_FILTER_UNSAFE_WORDS; i++) {
        range_filter_unsafe[i] = 0;
    }
    for (int32_t i = 0; i < PTO2_TENSORMAP_RANGE_FILTER_SLOTS; i++) {
        range_filter[i] = PTO2TensorMapRangeFilterSlot{};
    }
    for (int32_t i = 0; i < PTO2_TENSORMAP_EXACT_CACHE_SLOTS; i++) {
        exact_cache[i] = PTO2TensorMapExactCacheSlot{};
    }
