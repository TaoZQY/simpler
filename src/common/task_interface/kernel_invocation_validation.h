...
inline InvocationStatus validate_invocation_header(
    ...
    if (header.abi_version != SIMPLER_KERNEL_INVOCATION_ABI_VERSION || header.header_bytes != sizeof(header) ||
        header.reserved0 != 0 || header.reserved[0] != 0 || header.reserved[1] != 0 ||
        header.mode != SIMPLER_MODE_KERNEL || header.callable_id < 0 ||
