...
int validate(void *context, const KernelInvocationBinding &binding, const KernelResourceBinding &resources) noexcept {
    const auto &n = get(context);
    if (!n.aicore || !n.aicpu || !n.aicore_blocks || !n.aicpu_blocks || !span(n.aicore_args, n.aicore_args_bytes) ||
        n.aicpu_args != binding.packet.data || n.aicpu_args_bytes != binding.packet.size ||
        !n.validate_runtime_binding || !n.clear_regions || !n.clear_region_count ||
        !span(n.cancel.address, n.cancel.bytes) || n.cancel.bytes % sizeof(uint32_t) != 0 ||
    ...
    for (size_t i = 0; i < n.placeholder_count; ++i) {
        ...
        if (p.addrOffset != binding.placeholders[i].address_offset ||
            p.dataOffset != binding.placeholders[i].data_offset ||
            p.addrOffset < sizeof(SimplerKernelInvocationHeader) ||
            p.dataOffset < sizeof(SimplerKernelInvocationHeader) || p.addrOffset > n.aicpu_args_bytes ||
    ...
    return n.validate_runtime_binding(n.runtime_binding, binding, resources);
}
...
KernelLaunchResult launch_bound_kernel_native(
    KernelExecutionState &state, const KernelInvocationBinding &binding, const KernelNativeInvocation &native,
    void *caller_stream
) {
    NativeCall call{native};
    const KernelLaunchGateOps gate{&call, validate, query};
    const KernelLaunchOps ops{&call, wait, clear, record, cpu, core, cancel};
    return launch_bound_kernel(state, binding, caller_stream, gate, ops);
}
