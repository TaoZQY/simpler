...
int32_t hbg::build_graph(
    ...
    // region is committed by upload_graph at the measured size. compact_live_image
...
int32_t hbg::get_kernel_resource_requirements(
    const GraphBuild &build, const RuntimeArenaLayout &layout, KernelResourceRequirements &requirements
) {
    ...
    KernelResourceRequirements next{};
    ...
    uint64_t total = 0;
    if (!next.runtime_owned_bytes(total)) return PTO_RUNTIME_ERR_CAPACITY_EXCEEDED;
    requirements = next;
    return 0;
}

int32_t hbg::upload_graph(
...
int32_t run_host_orchestration(
    ...
    return hbg::upload_graph(runtime, api, rt, host_arena, layout, build);
}
