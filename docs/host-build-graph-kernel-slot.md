...
5. `prepare_graph_execution_slot(..., prepare_ops)` seals and calls the supplied
   ...
   The operation deep-copies the Host record before returning and enqueues
   `simpler_aicpu_l1_hbg_register_execution_slot`. That entry performs device
   initialization, registration and binding in order.
