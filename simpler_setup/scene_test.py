...
_AICPU_THREAD_NUM_ENV = "SIMPLER_AICPU_THREAD_NUM"
_BLOCK_DIM_ENV = "SIMPLER_BLOCK_DIM"
_PIPELINE_STRATEGY_DEFAULT = -1
...
def _resolve_aicpu_thread_num(default: int) -> int:
    """Return the case AICPU thread count, optionally overridden by env."""
    raw = os.environ.get(_AICPU_THREAD_NUM_ENV)
    if raw is None or raw == "":
        return default
    try:
        value = int(raw, 10)
    except ValueError:
        logger.warning("%s=%r is not an integer; using case value %d", _AICPU_THREAD_NUM_ENV, raw, default)
        return default
    if value <= 0:
        logger.warning("%s=%r must be positive; using case value %d", _AICPU_THREAD_NUM_ENV, raw, default)
        return default
    return value


def _resolve_block_dim(default: int) -> int:
    """Return the case block_dim, optionally overridden by env."""
    raw = os.environ.get(_BLOCK_DIM_ENV)
    if raw is None or raw == "":
        return default
    try:
        value = int(raw, 10)
    except ValueError:
        logger.warning("%s=%r is not an integer; using case value %d", _BLOCK_DIM_ENV, raw, default)
        return default
    if value <= 0:
        logger.warning("%s=%r must be positive; using case value %d", _BLOCK_DIM_ENV, raw, default)
        return default
    return value


def _resolve_pipeline_strategy(default: int) -> int:
    try:
        return int(default)
    except (TypeError, ValueError):
        logger.warning("pipeline_strategy=%r is not an integer; using default %d", default, _PIPELINE_STRATEGY_DEFAULT)
        return _PIPELINE_STRATEGY_DEFAULT
...
class SceneTestCase:
    ...
    def _build_config(
        ...
        *,
        ...
        config = CallConfig()
        config.block_dim = _resolve_block_dim(config_dict.get("block_dim", 1))
        config.aicpu_thread_num = _resolve_aicpu_thread_num(config_dict.get("aicpu_thread_num", 3))
        config.pipeline_strategy = _resolve_pipeline_strategy(
            config_dict.get("pipeline_strategy", _PIPELINE_STRATEGY_DEFAULT)
        )
